""" This program can be used to calculate value of a/b to n decimal digits
of precision given integers a, b, and n.
There are four main functions in this program:
(1) divide(a,b,n1) which calculates a/b to n1 decimal digits of precision
provided that gcd(b1,10) = 1 where b1 = b/gcd(a,b).
(2) digits(a,b,n1,n2) which calculates the n1 to n2 digits in the fractional
part of the decimal expansion of a/b assuming that gcd(b1,10) = 1 where b1
is defined in (1).
(3) divide2(a,b,n1) This is an improved version of the function divide that
works if neither 2^17 nor 5^17 divide b1 where b1 is defined in (1).
(4) digits2(a,b,n1,n2)  This returns a 2-tuple. The first number is equal to
how many digits have to be skipped in the decimal expansion of a/b until it
starts repeating and the second returned value is a string that is equal
to the n1 to n2 digits in that repeating part of a/b.
...
subtractive methods or multiplicative (iterative) methods are not efficient
or computationally feasible for large values of n1 (for example n1 > 2^100)
The output of the first three functions is returned as a string and the output
of the fourth is a 2-tuple that is comprised of an integer and a string.
See the comments in front of the definitions of all four functions
